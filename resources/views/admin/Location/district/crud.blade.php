                            ...
                            <div class="row">
                                ...
                                <div class="mb-3 col-md-6">
                                    ...
                                    <x-admin.form.input old="{{ 'name.'.$item->key }}"
                                        ...
                                        :value="$District->translate('name', $item->key)"></x-admin.form.input>
                                    <x-admin.form.label-end
                                        name="Please Enter {{ ucfirst($item->name) }} Admin Name">
                                    </x-admin.form.label-end>
                                </div>
                                @endforeach
                                @livewire('location',['CityId'=>$District->City->id ?? '','CountryId'=>$District->City->Country->id ?? ''])
                                <div class="mb-3 col-md-6">
                                    <x-admin.form.label-first star="*" name="{{ TranslationHelper::translate('State') }}"></x-admin.form.label-first>
                                    <x-admin.form.state :foreach="App\Models\StaticConst::STATUS" :model="$District" name="status"></x-admin.form.state>
